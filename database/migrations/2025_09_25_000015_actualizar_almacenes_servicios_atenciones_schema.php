...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasTable('almacenes_servicios_atenciones')) {
            Schema::create('almacenes_servicios_atenciones', function (Blueprint $table) {
                $table->id();
        ...
        Schema::table('almacenes_servicios_atenciones', function (Blueprint $table) {
            foreach (['insumos', 'codigo', 'numero_lote', 'fecha_vencimiento', 'fecha_ingreso'] as $columna) {
                if (Schema::hasColumn('almacenes_servicios_atenciones', $columna)) {
                    $table->dropColumn($columna);
                }
            }

            if (!Schema::hasColumn('almacenes_servicios_atenciones', 'cantidad')) {
                $table->unsignedInteger('cantidad')->default(0)->after('id');
            }
            if (!Schema::hasColumn('almacenes_servicios_atenciones', 'sede_id')) {
                $table->unsignedBigInteger('sede_id')->nullable()->after('cantidad');
                $table->index('sede_id');
            }
            if (!Schema::hasColumn('almacenes_servicios_atenciones', 'lote_id')) {
                $table->unsignedBigInteger('lote_id')->nullable()->after('sede_id');
                $table->index('lote_id');
            }
            if (!Schema::hasColumn('almacenes_servicios_atenciones', 'hospital_id')) {
                $table->unsignedBigInteger('hospital_id')->nullable()->after('lote_id');
                $table->index('hospital_id');
            }
            if (Schema::hasColumn('almacenes_servicios_atenciones', 'status')) {
